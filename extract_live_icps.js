...
async function extractICPs() {
  console.log('--- STARTING REVISED ICP EXTRACTION PIPELINE ---');

  // PHASE 1: Parse Appointments to lock in Geography & Carrier Count
  console.log('Phase 1: Streaming Appointments CSV (DFW Geography & 10+ Carriers)...');
  ...
  await new Promise((resolve, reject) => {
    fs.createReadStream(APPOINTMENTS_FILE)
      ...
      .on('end', () => {
        ...
        console.log(`Unique Agencies located in DFW Metroplex: ${validGeoAgencies.size}`);
        ...
        console.log(`DFW Agencies surviving 10+ Carrier Independence Filter: ${validGeoAgencies.size}`);
        resolve();
      })
      .on('error', reject);
  });
  ...
  console.log('\nPhase 2: Streaming Relationships CSV (7-30 Producer Filter)...');
  ...
  await new Promise((resolve, reject) => {
    fs.createReadStream(RELATIONSHIPS_FILE)
      ...
      .on('data', (row) => {
        ...
        // If the Licensee is one of our target DFW agencies, add the Associated Licensee as a producer
        ...
        // Or if the Associated Licensee is one of our target DFW agencies, add the Licensee as a producer
  ...
  for (const npn of validGeoAgencies) {
    ...
    // The Goldilocks Zone: 7 to 30 Producers
  ...
  console.log(`\n=================================================`);
  console.log(`FINAL RESULT: Mathematically verified True ICPs: ${finalICPs.length}`);
  console.log(`=================================================`);
  
  if (finalICPs.length > 0) {
    console.log("\nTop 10 ICP Targets Identified:");
    finalICPs.slice(0, 10).forEach((icp, index) => {
      console.log(`${index + 1}. ${icp.name} | ${icp.city}, ${icp.state} | ${icp.producer_count} Producers | ${icp.carrier_count} Carriers`);
    });
  }

  // Write to file
  const outputFile = 'live_icp_targets.json';
  fs.writeFileSync(outputFile, JSON.stringify(finalICPs, null, 2));
  console.log(`\nSaved complete hit-list to ${outputFile}`);
}
