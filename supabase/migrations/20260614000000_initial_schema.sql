...
CREATE TABLE users (
    ...
    home_agency_id UUID REFERENCES agencies(id) ON DELETE SET NULL, -- Identifies the Principal's own agency
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
SELECT 
    ...
    -- Get the names of those shared carriers
    (
        SELECT ARRAY_AGG(c.carrier_name)
        FROM agency_carrier_appointments aca1
        JOIN agency_carrier_appointments aca2 ON aca1.carrier_id = aca2.carrier_id
        JOIN carriers c ON c.id = aca1.carrier_id
        WHERE aca1.agency_id = a1.id AND aca2.agency_id = a2.id
          AND aca1.status = 'ACTIVE' AND aca2.status = 'ACTIVE'
    ) AS shared_carrier_names,
    -- Computed Score
...
-- Grant access to the API roles so Supabase Client can query and insert data
GRANT ALL ON ALL TABLES IN SCHEMA public TO anon, authenticated, service_role;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO anon, authenticated, service_role;
GRANT ALL ON ALL ROUTINES IN SCHEMA public TO anon, authenticated, service_role;

-- RPC to refresh the competitor view
CREATE OR REPLACE FUNCTION refresh_competitor_view()
RETURNS void AS $$
BEGIN
  REFRESH MATERIALIZED VIEW dynamic_competitor_discovery;
END;
$$ LANGUAGE plpgsql;
